...
client.basePath = 'http://localhost:8001/';

client.createClient = function (router) {
    ...
    if (path.length > 0) {
      ...
      if(x.siblings.length > 1) {
        ...
        if(typeof eval(str) === 'undefined') {
          ...
          str += ' = function () { client.request("'+ args.join('/') +'", "'+ verbs +'", arguments)};';
        }
        eval(str);
      }
...
client.request = function (path, verbs, args) {
  ...
  //
  var uri = client.basePath + path.join('/');

  //
  // Make an outgoing HTTP request using the `request` library
  //
  request({ uri: uri, method: verb }, function(err, res, body){
